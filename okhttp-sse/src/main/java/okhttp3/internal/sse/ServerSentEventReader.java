...
public final class ServerSentEventReader {
  ...
  public interface Callback {
    void onMessage(@Nullable String id, @Nullable String event, String data);
    void onRetryUpdated(long timeMs);
  }
  ...
  /**
   * Process the next event. This will result in a single call to {@link Callback#onMessage}
   * <em>unless</em> the data section was empty. Any number of calls to
   * {@link Callback#onRetryUpdated} may occur while processing an event.
   *
  ...
  boolean processNextEvent() throws IOException {
    ...
    while (true) {
      ...
      if (crlf == 0L) {
        ...
        if (data != null) {
          lastId = id;
          callback.onMessage(id, name, data.toString());
        }
      ...
      switch (fieldName) {
        ...
        case "retry":
          ...
          callback.onRetryUpdated(timeMs);
          break;
