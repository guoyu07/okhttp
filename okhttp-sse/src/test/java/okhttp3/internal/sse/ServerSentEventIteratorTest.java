...
public final class ServerSentEventIteratorTest {
  /** Either {@link Event} or {@link Long} items for events and retry updates, respectively. */
  ...
  private void consumeEvents(String source) throws IOException {
    ServerSentEventReader.Callback callback = new ServerSentEventReader.Callback() {
      @Override public void onMessage(@Nullable String id, @Nullable String event, String data) {
        callbacks.add(new Event(id, event, data));
      }
      @Override public void onRetryUpdated(long timeMs) {
